...
int UltraFace::detect(cv::Mat &raw_image, std::vector<FaceInfo> &face_list)const
{
    if (raw_image.empty()) {
    ...
    generateBBox(bbox_collection, tensor_scores, tensor_boxes,cv::Size(raw_image.cols,raw_image.rows));
    nms(bbox_collection, face_list);
    return 0;
}

void UltraFace::generateBBox(std::vector<FaceInfo> &bbox_collection, MNN::Tensor *scores, MNN::Tensor *boxes, cv::Size size)const
{
    for (int i = 0; i < num_anchors; i++) {
        if (scores->host<float>()[i * 2 + 1] > score_threshold) {
            ...
            rects.x1 = clip(x_center - w / 2.0, 1) * size.width;
            rects.y1 = clip(y_center - h / 2.0, 1) * size.height;
            rects.x2 = clip(x_center + w / 2.0, 1) * size.width;
            rects.y2 = clip(y_center + h / 2.0, 1) * size.height;
